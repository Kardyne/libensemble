...
class CuttrJobController(JobController):
    ...
    def __init__(self, registry=None, auto_resources=True, nodelist_env_slurm=None, nodelist_env_cobalt=None):
        if MPI.COMM_WORLD.Get_rank() == 0:
            ...
            cpulists = subprocess.check_output('hwloc-distrib {} | hwloc-calc --li --po --pulist'.format(slice_count), shell=True).strip().split('\n') * int(math.ceil(float(worker_count) / float(slice_count)))
            numanodes = subprocess.check_output('hwloc-distrib {} | hwloc-calc --li --po --pulist --intersect NUMAnode'.format(slice_count), shell=True).strip().split('\n') * int(math.ceil(float(worker_count) / float(slice_count)))
